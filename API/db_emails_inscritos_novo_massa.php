...
if ($d_enviados == "TODOS"){
$LISTA = mysqli_query ($connect, "SELECT * FROM emails_inscritos WHERE status = 'RECEBER_EMAILS' ORDER BY id DESC LIMIT ".$inicio.",".$fim."");
$LISTA_T = mysqli_query ($connect, "SELECT * FROM emails_inscritos WHERE status = 'RECEBER_EMAILS' ORDER BY id DESC");
$LISTA_TOTAL = mysqli_num_rows($LISTA_T);
}else
if ($d_enviados == "NUNCA"){
$LISTA = mysqli_query ($connect, "SELECT * FROM emails_inscritos WHERE status = 'RECEBER_EMAILS' AND enviados = '0' ORDER BY id DESC LIMIT ".$inicio.",".$fim."");
$LISTA_T = mysqli_query ($connect, "SELECT * FROM emails_inscritos WHERE status = 'RECEBER_EMAILS' AND enviados = '0' ORDER BY id DESC");;
$LISTA_TOTAL = mysqli_num_rows($LISTA_T);
}else
if ($d_enviados == "JA_RECEBEU"){
$LISTA = mysqli_query ($connect, "SELECT * FROM emails_inscritos WHERE status = 'RECEBER_EMAILS' AND enviados >= '1' ORDER BY id DESC LIMIT ".$inicio.",".$fim."");
$LISTA_T = mysqli_query ($connect, "SELECT * FROM emails_inscritos WHERE status = 'RECEBER_EMAILS' AND enviados >= '1' ORDER BY id DESC");
$LISTA_TOTAL = mysqli_num_rows($LISTA_T);
}
...
$TEXTO = str_replace("(ID)", $LISTAS['id'], str_replace("(EMAIL)", $LISTAS['email'], $d_texto));
...
$ENVIADOS = $LISTAS['enviados']+1;
if (mysqli_query($connect, "UPDATE `emails_inscritos` SET `enviados`= '".$ENVIADOS."' WHERE id = '".$LISTAS['id']."'")) {
...
window.location=\'https://www.fourweeks.com.br/API/db_emails_inscritos_novo_massa.php'.$POSTS_GETS.'\'; 
</script> 
 
<noscript> 
Se não for direcionado automaticamente, clique <a href=\'https://www.fourweeks.com.br/API/db_emails_inscritos_novo_massa.php'.$POSTS_GETS.'\'>aqui</a>. 
</noscript>
';
header('Location: https://www.fourweeks.com.br/API/db_emails_inscritos_novo_massa.php'.$POSTS_GETS);
