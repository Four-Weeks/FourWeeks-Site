...
<div class="container-fluid">
  ...
  <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
    ...
    <ul class="nav navbar-nav navbar-right">
      <li><a href="liadwa.php">Adicinar novo</a></li>
    </ul>
  </div><!-- /.navbar-collapse -->
</div><!-- /.container-fluid -->
</nav>
...
if ($_GET['p'] != null){
$LISTA = mysqli_query ($connect, "SELECT * FROM contatos WHERE id = '".anti_inje($_GET['p'])."' OR nome LIKE '%".anti_inje($_GET['p'])."%' OR email LIKE '%".anti_inje($_GET['p'])."%' ORDER BY id DESC LIMIT 200");
}else{
$LISTA = mysqli_query ($connect, "SELECT * FROM contatos ORDER BY id DESC LIMIT 200");
}
...
<td>'.$LISTAS['nome'].'</td>
<td>'.$LISTAS['email'].'</td>
<td>'.$LISTAS['profissao'].'</td>
<td>'.$LISTAS['dataNascimento'].'</td>
<td>'.$LISTAS['password'].'</td>
<td>'.$LISTAS['progresso'].'</td>
<td>'.$LISTAS['licao'].'</td>
<td>'.$LISTAS['momento'].'</td>
</tr>
';
}
